    ...
    private void validate() {
        ...
        if (!isPrinting && calibrate == null) {
            throw new IllegalArgumentException(
                "[DazaiKindle] ❌ Calibration is mandatory.\n" +
                "[DazaiKindle]    Provide --calibrate <file>, or run --print-calibration-template <path>."
            );
        }
        if (!isPrinting && calibrate != null && !calibrate.exists()) {
            throw new IllegalArgumentException(
                "[DazaiKindle] ❌ Calibration file not found: " + calibrate.getPath()
            );
        }
        if (!isPrinting && !headingsOnly) {
            if (clippings == null) {
                throw new IllegalArgumentException("[DazaiKindle] ❌ Missing --clippings.");
            }
            if (template == null) {
                throw new IllegalArgumentException("[DazaiKindle] ❌ Missing --template.");
            }
        }
    }
