...
import io.github.vlovric.dazaikindle.calibration.CalibrationFit;
...
public class FitCalibrationStep implements PipelineStep {
    ...
    public StepResult execute(PipelineContext ctx) throws Exception {
        ...
        System.out.printf(
            "[DazaiKindle] 📐 Calibration: bytesPerLocation=%.2f, bias=%.2f, points=%d, rmse=%.2f locations%n",
            fit.bytesPerLocation(), fit.locationBias(), fit.pointsUsed(), fit.rmseLocations()
        );

        if (fit.rmseLocations() > RMSE_WARNING_THRESHOLD) {
            System.out.printf(
                "[DazaiKindle] ⚠️  High calibration RMSE (%.2f > %.1f). Results may be inaccurate — add more calibration points.%n",
