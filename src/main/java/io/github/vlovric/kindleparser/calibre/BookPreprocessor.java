...
public class BookPreprocessor {
    ...
    public static Path preprocess(Path inputFile) throws IOException, InterruptedException {
        if (!Files.exists(inputFile)) {
            throw new IOException("[KindleParser] ❌ Book file not found: " + inputFile.toAbsolutePath());
        }
        ...
        if (!SUPPORTED_EXTENSIONS.contains(extension)) {
            throw new IllegalArgumentException(
                "[KindleParser] ❌ Unsupported file format: '" + extension + "'. Supported formats: .epub, .azw3, .mobi"
            );
        }
    ...
    private static Path convertToEpub(Path inputFile) throws IOException, InterruptedException {
        ...
        if (Files.exists(epubPath)) {
            System.out.println("[KindleParser] EPUB already exists, skipping conversion: " + epubPath.toAbsolutePath());
            return epubPath;
        }

        System.out.println("[KindleParser] Converting " + filename + " to EPUB via Calibre...");
        ...
        try {
            ...
            throw new IOException(
                "[KindleParser] ❌ Could not launch Calibre's ebook-convert. Is Calibre installed and in PATH?", e
            );
        }
        ...
        try {
            ...
            throw new InterruptedException("[KindleParser] ❌ Calibre conversion was interrupted.");
        }

        if (exitCode != 0) {
            throw new IOException(
                "[KindleParser] ❌ ebook-convert failed (exit code " + exitCode + "). Check that the file is a valid ebook and that Calibre has write permission to: " + epubPath.getParent()
            );
        }

        System.out.println("[KindleParser] ✅ Converted to: " + epubPath.toAbsolutePath());
        return epubPath;
    }
