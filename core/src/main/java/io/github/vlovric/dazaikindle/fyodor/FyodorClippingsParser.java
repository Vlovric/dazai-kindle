...
public class FyodorClippingsParser {
    ...
    /**
     * Ensures {@code template.erb} in {@code configDir} matches the bundled DazaiKindle template.
     * <ul>
            ...
            if (!normalize(existing).equals(normalize(bundled))) {
                if (!overwriteFyodorTemplate) {
                    throw new IOException(
                            "[DazaiKindle] ❌ ~/.config/fyodor/template.erb exists but has different content "
                            + "than the required DazaiKindle template.\n"
                            + "[DazaiKindle]    Re-run with --overwrite-fyodor-template to replace it.");
                }
                Files.writeString(templatePath, bundled, StandardCharsets.UTF_8);
                System.out.println("[DazaiKindle] ⚠️  Overwrote existing Fyodor template at " + templatePath.toAbsolutePath());
            }
        } else {
            Files.writeString(templatePath, bundled, StandardCharsets.UTF_8);
            System.out.println("[DazaiKindle] ✅ Fyodor template saved to " + templatePath.toAbsolutePath());
        }
    }
    ...
    private void checkUserFyodorToml() {
        try {
            ...
            if (!Files.exists(toml)) {
                System.err.println("[DazaiKindle] ⚠️  Missing ~/.config/fyodor/fyodor.toml. Fyodor may not write the expected *.json output filenames.");
            }
